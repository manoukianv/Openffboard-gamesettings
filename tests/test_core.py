...
class TestSimRacingGames(unittest.TestCase):
    ...
    @patch("offbgamessettings.game_discovery._get_steam_path_windows", return_value="C:\\Steam")
    @patch("offbgamessettings.game_discovery._get_steam_path_linux", return_value="/home/user/.steam/steam")
    ...
    @patch("offbgamessettings.game_discovery.find_steam_path", return_value="/fake/steam")
    @patch("os.path.exists")
    @patch("builtins.open", new_callable=mock_open)
    @patch("os.listdir")
    @patch("os.path.isdir")
    def test_get_sim_racing_game_folders(self, mock_isdir, mock_listdir, mock_open_func, mock_exists, mock_find_steam):
        ...
        mock_open_func.side_effect = [
            mock_open(read_data=MOCK_LIBRARYFOLDERS_VDF).return_value,  # libraryfolders.vdf
            mock_open(read_data=MOCK_APPMANIFEST_244210_ACF).return_value, # appmanifest_244210.acf in first library
            mock_open(read_data=MOCK_APPMANIFEST_244210_ACF).return_value  # appmanifest_244210.acf in second library
        ]
        ...
        expected_folders = {
            "244210": {
                "name": "Assetto Corsa",
                "path": os.path.join("/fake/steam", "steamapps", "common", "assettocorsa")
            }
        }
        ...
        mock_open_func.assert_any_call(os.path.join("/fake/steam", "steamapps", "libraryfolders.vdf"), "r", encoding="utf-8")
        mock_open_func.assert_any_call(os.path.join("/fake/steam", "steamapps", "appmanifest_244210.acf"), "r", encoding="utf-8")
