...
class DirtWrcConfigurator(BaseGameConfigurator):
    ...
    def check_and_configure(self):
        ...
        if not device_defines_path or not os.path.exists(device_defines_path):
            self.logs.append({"status": "WARNING", "message": "Could not find device_defines.xml. The game may be corrupt or incomplete."})
            self.status = "WARNING"
        ...
        try:
            ...
            if not root.findall(f".//device[@id='{device_id}']"):
                ...
                if backup_file(device_defines_path):
                    ...
                    self.logs.append({"status": "MODIFIED", "message": "OpenFFBoard device added to device_defines.xml."})
            ...
            self.logs.append({"status": "ERROR", "message": f"Failed to parse {os.path.basename(device_defines_path)}. The file may be corrupt."})
            self.status = "ERROR"
            ...
            if not os.path.exists(openffboard_xml_path):
                ...
                try:
                    with open(openffboard_xml_path, "w", encoding="utf-8") as f:
                        f.write(xml_content)
                    self.logs.append({"status": "MODIFIED", "message": "Action map file openffboard.xml created."})
                    if self.status != "ERROR": self.status = "MODIFIED"
                except IOError:
                    self.logs.append({"status": "ERROR", "message": "Could not write to openffboard.xml."})
                    self.status = "ERROR"
            else:
                self.logs.append({"status": "OK", "message": "Action map file openffboard.xml already exists."})
        else:
    ...
    def revert_configuration(self):
        ...
        if os.path.exists(backup_path):
            try:
                ...
                self.logs.append({"status": "RESTORED", "message": f"{os.path.basename(device_defines_path)} restored from backup."})
